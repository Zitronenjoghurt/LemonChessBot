...
def get_routines(bot: commands.AutoShardedBot) -> list[tuple[int, Callable]]:
    routines = []
    for file_name in files_in_directory(ROUTINES_PATH, ".py"):
        ...
        routines.append((interval, create_run_method(
            bot=bot, module=module, module_name=module_name)))
    return routines

def create_run_method(bot: commands.AutoShardedBot, module, module_name: str) -> Callable:
    async def run():
